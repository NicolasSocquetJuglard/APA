...
val MIGRATION = object : Migration(6, 7) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("CREATE TABLE IF NOT EXISTS answerExercisesAPA (answerExercisesAPAId INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, nb_exercises INTEGER NOT NULL, difficulty INTEGER NOT NULL, pain INTEGER NOT NULL, sessionId INTEGER NOT NULL, FOREIGN KEY (sessionId) REFERENCES sessions(sessionId) ON DELETE CASCADE);")
        /*
        database.execSQL("ALTER TABLE answerExercises RENAME COLUMN nb_exercices TO nb_exercises")
        database.execSQL("ALTER TABLE answerExercises RENAME COLUMN difficulte TO difficulty")
        database.execSQL("ALTER TABLE answerExercises RENAME COLUMN douleur TO pain")
...
@Database(entities = [User::class, Session::class, AnswerExercisesAPA::class, AnswerRefusal::class, Physio::class, ExercisesSheet::class, MessagesEncouragement::class], version = 7)
        ...
        fun getInstance(context: Context): AppDatabase{
            return INSTANCE ?: synchronized(this){
                val instance = Room.databaseBuilder(
                    ...
                    "app_database").addMigrations(MIGRATION).allowMainThreadQueries().build()
                INSTANCE = instance
                instance
            }
        }
